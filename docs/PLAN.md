...
> ⚠️ **먼저 읽을 것**: 연락처에 저장된 번호에는 팝업이 뜨지 않는 플랫폼 제약이 있다.
> 실기기에서 확인됐고 아직 해결되지 않았다. → [7. 미해결 이슈](#7-미해결-이슈)
...
### 7-1. 연락처에 저장된 번호에는 팝업이 뜨지 않는다 (미해결, 설계 영향 큼)

**증상**
전화가 와도 팝업이 뜨지 않고 수신 이력에도 남지 않는다. 그 번호를 기기 연락처에서
지우면 즉시 정상 동작한다. (2026-07-31 Galaxy S24 Ultra / Android 16 실기기 확인)

**원인**
우리 코드 문제가 아니라 플랫폼 동작이다. 안드로이드 공식 문서에 명시돼 있다.

> `onScreenCall()` is called for any new incoming or outgoing calls
> **when the number is not in the user's contact list**
> — [Screen calls | Android Developers](https://developer.android.com/develop/connectivity/telecom/dialer-app/screen-calls)

`CallScreeningService` 는 스팸 차단용으로 설계됐고, "이미 아는 사람"은 스크리닝
대상에서 제외하는 게 기본 정책이다. 연락처에 있으면 콜백 자체가 오지 않으므로
우리 코드가 실행될 기회가 없다. 로그·권한·설정으로 우회할 수 있는 종류가 아니다.

**왜 치명적인가**
이 앱은 *직원*을 알아보는 게 목적인데, 직원은 대부분 이미 기기 연락처에 있다.
즉 가장 중요한 사용 사례에서 정확히 동작하지 않는다. 2장의 기술 검토에서
이 제약을 짚지 못한 것이 이 플랜의 가장 큰 누락이다.

**선택지**

| | 방식 | 연락처 저장된 사람 | 대가 |
|---|---|---|---|
| A | **연락처 동기화** — 직원 정보를 전용 계정 연락처로 써넣어 삼성 통화화면이 이름·부서·직급을 직접 띄우게 한다 | 됨 | 커스텀 팝업이 아니다. 메모·태그·이력은 못 보여준다. 사용자 연락처를 건드린다(별도 계정이라 분리·일괄 삭제는 가능) |
| B | **`READ_CALL_LOG` + `PHONE_STATE` 브로드캐스트**를 두 번째 감지 경로로 추가 | 됨 | Play Store 정책상 거의 반려(기본 전화앱/발신자표시앱만 허용). 사내 배포면 무관. **Android 16 에서 `EXTRA_INCOMING_NUMBER` 가 아직 전달되는지 미검증** |
| C | **기본 전화앱(`InCallService`)** — 통화 화면을 통째로 그린다 | 됨 | 삼성 전화앱 대체(통화 녹음 등 상실), 공수 2~3배 |
| D | 알림 접근으로 전화앱 알림 파싱 | 됨 | 비공식. One UI 업데이트마다 깨질 수 있다 |

**권장**: B를 주 경로로, A를 보조로. 지금 만든 팝업·명부·이력을 그대로 살릴 수 있고
사내 배포라면 정책 리스크가 없다. 다만 **B는 착수 전에 최소 구현으로 실기기
30초 검증**이 필요하다. `EXTRA_INCOMING_NUMBER` 가 막혀 있으면 B는 무효이고 C로 간다.

**결정 대기 중**: Play Store 배포 여부. 스토어로 가면 B가 불가능해 A로 가야 하고,
그 경우 기능 정의가 "팝업"에서 "통화화면에 이름이 뜬다"로 바뀐다.

### 7-2. 이미 고친 것 (참고)

`onScreenCall` 에서 `callDirection != DIRECTION_INCOMING` 이면 수신 이력을 남기기 전에
빠져나가고 있었다. 기기에 따라 방향이 `DIRECTION_UNKNOWN` 으로 오는 경우가 있어
8-1과 증상이 겹쳐 원인 판별을 어렵게 만들었다. 명시적으로 발신일 때만 건너뛰도록
바꾸고, 콜백 수신 사실 자체를 기록하는 "통화 감지 진단" 카드를 동작 준비 화면에 넣었다.

---

## 8. 명부를 구글 시트로 관리하기

여러 대의 폰에 같은 명부를 넣어야 하고, 편집은 앱보다 시트가 훨씬 편하다.
그래서 **시트를 원본으로 두고 앱이 읽어 가는** 단방향 동기화로 간다.

**인증 방식 선택**
- **채택: 공개 링크 CSV 내보내기.** 시트를 "링크가 있는 사용자 - 뷰어"로 열어 두고
  `.../export?format=csv&gid=…` 를 그냥 받아온다. GCP 프로젝트도, OAuth 동의 화면도,
  서명키 SHA-1 등록도 필요 없다. 앱을 받자마자 URL만 붙여넣으면 끝난다.
- 기각: Sheets API + OAuth. 정석이지만 관리자가 GCP 프로젝트를 만들고 OAuth 클라이언트에
  릴리스 키 지문을 등록해야 앱이 빌드조차 의미 없어진다. 사내 도구에 물리기엔 과하다.

**⚠️ 대가**: 링크를 아는 사람은 누구나 시트를 볼 수 있다. 직원 이름·전화번호가
들어가므로 링크 관리가 곧 보안이다. 사내 전용이면 감수할 만하지만, 외부 유출이
곤란한 명부라면 OAuth(위 기각안)로 올려야 한다. 이건 나중에 갈아끼울 수 있게
`SheetDirectoryClient` 뒤에 격리해 둔다.

**동기화 규칙**
- 시트에서 온 직원과 앱에서 직접 만든 직원을 `source` 로 구분한다.
- 시트 행의 안정적인 키(`사번` → 없으면 첫 번호 → 없으면 이름)로 매칭해 갱신한다.
  키가 안정적이어야 시트에서 부서를 고쳐도 새 사람으로 취급되지 않는다.
- 시트에서 사라진 행은 지운다. 단 **앱에서 직접 만든 직원은 건드리지 않는다.**
- 실패해도 기존 명부는 그대로 둔다. 전화가 왔을 때 이름이 안 뜨는 것보다
  옛 이름이라도 뜨는 게 낫다.

**나중에 할 것**: 양방향(앱 편집 → 시트 반영)은 충돌 해결이 필요해 범위 밖.
지금은 시트가 단일 원본이고 앱은 읽기 전용이다.
